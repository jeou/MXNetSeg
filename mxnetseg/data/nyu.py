...
class NYUSegmentation(SegmentationDataset):
    """ NYUv2 semantic segmentation dataset (with 40 semantic classes).
    N. Silberman, et al. Indoor segmentation and support inference from rgbd images. CVPR 2012.
    S. Gupta, et al. Perceptual organization and recognition of indoor scenes from rgb-d images. CVPR 2013.
    """
    NUM_CLASS = 40

    def __init__(self, root=os.path.expanduser('~/.mxnet/dataset/NYU'), split='train',
                 mode=None, transform=None, **kwargs):
        super(NYUSegmentation, self).__init__(root, split, mode, transform, **kwargs)
        _img_dir = os.path.join(root, 'images')
        _mask_dir = os.path.join(root, 'labels_40')
        if split == 'train':
    ...
    @property
    def classes(self):
        return ('wall', 'floor', 'cabinet', 'bed', 'chair', 'sofa', 'table', 'door', 'window', 'bookshelf',
