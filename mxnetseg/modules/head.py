...
class FCNHead(nn.HybridBlock):
    ...
    def __init__(self, nclass, in_channels=0, norm_layer=nn.BatchNorm, norm_kwargs=None,
                 activation='relu', drop_out=.1, reduction=4):
        super(FCNHead, self).__init__()
        inter_channels = 256 if in_channels == 0 else in_channels // reduction
...
class CARAFE(nn.HybridBlock):
    """
    CARAFE up-sampling operator.
    Reference: Wang, J., Chen, K., Xu, R., Liu, Z., Loy, C. C., & Lin, D. (2019).
        CARAFE: Content-Aware ReAssembly of FEatures.
        In IEEE International Conference on Computer Vision.
    Adapted from: https://github.com/XiaLiPKU/CARAFE/blob/master/carafe.py
    """
