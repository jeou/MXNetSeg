...
def get_demo_image_path(name: str, tag: str = 'affinity') -> str:
    ...
    """
    if name not in _available_images(os.path.join(demo_path(), tag)):
        raise FileNotFoundError(f"Image does not exist: {name}")

    image_path_png = os.path.join(demo_path(), tag, f'{name}.png')
    image_path_jpg = os.path.join(demo_path(), tag, f'{name}.jpg')
